...
public class JoueurListController {
    ...
    private static final String AGE_FILTER_UNDER_18 = "Moins de 18 ans";
    private static final String AGE_FILTER_18_21 = "18 - 21 ans";
    private static final String AGE_FILTER_22_25 = "22 - 25 ans";
    private static final String AGE_FILTER_26_30 = "26 - 30 ans";
    private static final String AGE_FILTER_OVER_30 = "31 ans et plus";
    ...
    @FXML
    private ComboBox<String> positionFilterComboBox;
    @FXML
    private ComboBox<String> ageFilterComboBox;
    @FXML
    private DatePicker dateNaissanceFilterPicker;
    @FXML
    ...
    private void handleResetFilters() {
        ...
        if (positionFilterComboBox != null) {
            positionFilterComboBox.getSelectionModel().clearSelection();
        }
        if (ageFilterComboBox != null) {
            ageFilterComboBox.getSelectionModel().clearSelection();
        }
        if (dateNaissanceFilterPicker != null) {
            dateNaissanceFilterPicker.setValue(null);
        }
        applyFilters();
    ...
    private void configureAdvancedFilters() {
        if (positionFilterComboBox != null) {
            positionFilterComboBox.setPromptText("Tous les postes");
            positionFilterComboBox.valueProperty().addListener((observable, oldValue, newValue) -> applyFilters());
        }
        if (ageFilterComboBox != null) {
            ageFilterComboBox.getItems().setAll(
                    AGE_FILTER_UNDER_18,
                    AGE_FILTER_18_21,
                    AGE_FILTER_22_25,
                    AGE_FILTER_26_30,
            ...
            ageFilterComboBox.setPromptText("Tous les ages");
            ageFilterComboBox.valueProperty().addListener((observable, oldValue, newValue) -> applyFilters());
        }
        if (dateNaissanceFilterPicker != null) {
            dateNaissanceFilterPicker.valueProperty().addListener((observable, oldValue, newValue) -> applyFilters());
        }
    }
    ...
    private void applyFilters() {
        ...
        String positionFilter = normalize(positionFilterComboBox == null ? null : positionFilterComboBox.getValue());
        String ageFilter = ageFilterComboBox == null ? null : ageFilterComboBox.getValue();
        LocalDate birthDateFilter = dateNaissanceFilterPicker == null ? null : dateNaissanceFilterPicker.getValue();
        Integer queryNumber = extractNumericQuery(query);
        LocalDate queryDate = parseDateQuery(query);
        ...
        filteredJoueurs.setPredicate(joueur -> {
            boolean matchesQuery = query == null
                    ...
                    || containsNormalized(resolveBirthDateSearchValue(joueur), query)
                    || matchesNumericQuery(joueur, queryNumber)
                    || matchesDateQuery(joueur, queryDate);

            boolean matchesEquipe = filterEquipeId == null || Objects.equals(joueur.getEquipeId(), filterEquipeId);
            boolean matchesPosition = positionFilter == null
                    || Objects.equals(normalize(joueur.getPosition()), positionFilter);
            boolean matchesAge = ageFilter == null || matchesAgeFilter(joueur.getDateNaissance(), ageFilter);
            boolean matchesBirthDate = birthDateFilter == null || Objects.equals(joueur.getDateNaissance(), birthDateFilter);
            return matchesQuery && matchesEquipe && matchesPosition && matchesAge && matchesBirthDate;
        });
    ...
    private void updateSelectionState() {
        boolean hasFilters = normalize(searchField.getText()) != null
                || getSelectedFilterEquipeId() != null
                || (positionFilterComboBox != null && normalize(positionFilterComboBox.getValue()) != null)
                || (ageFilterComboBox != null && ageFilterComboBox.getValue() != null)
                || (dateNaissanceFilterPicker != null && dateNaissanceFilterPicker.getValue() != null);
        selectionStateLabel.setText(hasFilters
                ? "Recherche intelligente active"
                : "Cliquez sur une carte pour ouvrir la fiche");
    }
    ...
    private void updateActionAvailability() {
        ...
        if (positionFilterComboBox != null) {
            positionFilterComboBox.setDisable(disabled);
        }
        if (ageFilterComboBox != null) {
            ageFilterComboBox.setDisable(disabled);
        }
        if (dateNaissanceFilterPicker != null) {
            dateNaissanceFilterPicker.setDisable(disabled);
        }
        joueurListView.setDisable(disabled);
    }

    private void rebuildPositionFilterItems() {
        if (positionFilterComboBox == null) {
            return;
        }

        String selectedPosition = positionFilterComboBox.getValue();
        List<String> positions = joueurs.stream()
                .map(Joueur::getPosition)
        ...
        positionFilterComboBox.getItems().setAll(positions);
        if (selectedPosition != null && positions.contains(selectedPosition)) {
            positionFilterComboBox.setValue(selectedPosition);
    ...
    private String resolveBirthDateSearchValue(Joueur joueur) {
        if (joueur == null || joueur.getDateNaissance() == null) {
            return null;
        }
        return formatDate(joueur.getDateNaissance()) + " " + joueur.getDateNaissance();
    }

    private boolean matchesAgeFilter(LocalDate dateNaissance, String ageFilter) {
        if (dateNaissance == null || ageFilter == null) {
            return false;
        }

        int age = Period.between(dateNaissance, LocalDate.now()).getYears();
        return switch (ageFilter) {
            case AGE_FILTER_UNDER_18 -> age < 18;
            case AGE_FILTER_18_21 -> age >= 18 && age <= 21;
            case AGE_FILTER_22_25 -> age >= 22 && age <= 25;
            case AGE_FILTER_26_30 -> age >= 26 && age <= 30;
            case AGE_FILTER_OVER_30 -> age >= 31;
            default -> true;
        };
    }

    private boolean matchesNumericQuery(Joueur joueur, Integer queryNumber) {
        if (joueur == null || queryNumber == null) {
            return false;
        }

        if (joueur.getNumero() == queryNumber) {
            return true;
        }
        if (joueur.getDateNaissance() == null) {
            return false;
        }
        return Period.between(joueur.getDateNaissance(), LocalDate.now()).getYears() == queryNumber;
    }

    private boolean matchesDateQuery(Joueur joueur, LocalDate queryDate) {
        return joueur != null
                && queryDate != null
                && Objects.equals(joueur.getDateNaissance(), queryDate);
    }

    private Integer extractNumericQuery(String query) {
        if (query == null) {
            return null;
        }
    ...
    private LocalDate parseDateQuery(String query) {
        if (query == null) {
            return null;
        }
        ...
        try {
            ...
            // Try the UI date format next.
        }

        try {
            return LocalDate.parse(query, DATE_FORMATTER);
        } catch (Exception ignored) {
            return null;
        }
    }
