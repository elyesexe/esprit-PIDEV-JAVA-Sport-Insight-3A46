...
public class AnnonceController {
    ...
    private void configureTables() {
        ...
        annonceTableView.setEditable(true);
        ...
        annonceIdColumn.setEditable(false);
        annonceCommentairesColumn.setEditable(false);

        annonceTitreColumn.setEditable(true);
        annonceTitreColumn.setCellFactory(TextFieldTableCell.forTableColumn());
        annonceTitreColumn.setOnEditCommit(event -> {
            Annonce annonce = event.getRowValue();
            if (annonce == null) {
                return;
            }
            String newTitle = emptyToNull(event.getNewValue());
            String oldTitle = annonce.getTitre();
            if (newTitle == null) {
                showAnnonceValidation("Title is required.");
                annonceTableView.refresh();
                return;
            }
            annonce.setTitre(newTitle);
            persistInlineAnnonceUpdate(annonce, () -> annonce.setTitre(oldTitle), "Announcement title updated.");
        });

        annoncePosteColumn.setEditable(true);
        annoncePosteColumn.setCellFactory(TextFieldTableCell.forTableColumn());
        annoncePosteColumn.setOnEditCommit(event -> {
            Annonce annonce = event.getRowValue();
            if (annonce == null) {
                return;
            }
            String oldRole = annonce.getPosteRecherche();
            annonce.setPosteRecherche(emptyToNull(event.getNewValue()));
            persistInlineAnnonceUpdate(annonce, () -> annonce.setPosteRecherche(oldRole), "Announcement role updated.");
        });

        annonceNiveauColumn.setEditable(true);
        annonceNiveauColumn.setCellFactory(TextFieldTableCell.forTableColumn());
        annonceNiveauColumn.setOnEditCommit(event -> {
            Annonce annonce = event.getRowValue();
            if (annonce == null) {
                return;
            }
            String oldLevel = annonce.getNiveauRequis();
            annonce.setNiveauRequis(emptyToNull(event.getNewValue()));
            persistInlineAnnonceUpdate(annonce, () -> annonce.setNiveauRequis(oldLevel), "Announcement level updated.");
        });

        annonceDateColumn.setEditable(true);
        annonceDateColumn.setCellFactory(TextFieldTableCell.forTableColumn());
        annonceDateColumn.setOnEditCommit(event -> {
            Annonce annonce = event.getRowValue();
            if (annonce == null) {
                return;
            }
            String newDateText = emptyToNull(event.getNewValue());
            if (newDateText == null) {
                showAnnonceValidation("Publication date is required (dd/MM/yyyy).");
                annonceTableView.refresh();
                return;
            }
            ...
            try {
                parsedDate = LocalDate.parse(newDateText, DATE_FORMATTER);
            } catch (Exception ex) {
                showAnnonceValidation("Invalid date format. Use dd/MM/yyyy.");
                annonceTableView.refresh();
                return;
            }

            LocalDate oldDate = annonce.getDatePublication();
            annonce.setDatePublication(parsedDate);
            persistInlineAnnonceUpdate(annonce, () -> annonce.setDatePublication(oldDate), "Announcement date updated.");
        });

        annonceStatutColumn.setEditable(true);
        annonceStatutColumn.setCellFactory(ComboBoxTableCell.forTableColumn(FXCollections.observableArrayList(ANNONCE_STATUTS)));
        annonceStatutColumn.setOnEditCommit(event -> {
            Annonce annonce = event.getRowValue();
            if (annonce == null) {
                return;
            }
            String newStatus = emptyToNull(event.getNewValue());
            if (newStatus == null || ANNONCE_STATUTS.stream().noneMatch(status -> status.equalsIgnoreCase(newStatus))) {
                showAnnonceValidation("Choose a valid status.");
                annonceTableView.refresh();
                return;
            }
            String oldStatus = annonce.getStatut();
            annonce.setStatut(newStatus.toUpperCase(Locale.ROOT));
            persistInlineAnnonceUpdate(annonce, () -> annonce.setStatut(oldStatus), "Announcement status updated.");
        });

        annonceUrgentColumn.setEditable(true);
        annonceUrgentColumn.setCellFactory(ComboBoxTableCell.forTableColumn(FXCollections.observableArrayList("Urgent", "Normal")));
        annonceUrgentColumn.setOnEditCommit(event -> {
            Annonce annonce = event.getRowValue();
            if (annonce == null) {
                return;
            }
            ...
            if (value == null) {
                showAnnonceValidation("Choose priority: Urgent or Normal.");
                annonceTableView.refresh();
                return;
            }

            Boolean oldUrgent = annonce.getUrgent();
            if ("URGENT".equalsIgnoreCase(value)) {
                annonce.setUrgent(true);
            } else if ("NORMAL".equalsIgnoreCase(value)) {
                annonce.setUrgent(false);
            } else {
                showAnnonceValidation("Choose priority: Urgent or Normal.");
                annonceTableView.refresh();
                return;
            }
            persistInlineAnnonceUpdate(annonce, () -> annonce.setUrgent(oldUrgent), "Announcement priority updated.");
        });
    ...
    private Commentaire buildCommentaireFromForm(boolean updateMode) {
        ...
        return new Commentaire(
                ...
                annonceId,
                auteur,
                selectedCommentaire == null ? null : selectedCommentaire.getCvName(),
                likes == null ? 0 : likes,
                moderationStatus,
                moderationReason
        );
    ...
    private void persistInlineAnnonceUpdate(Annonce annonce, Runnable rollback, String successMessage) {
        clearAnnonceValidation();
        if (annonce == null || annonceService == null) {
            if (rollback != null) {
                rollback.run();
        ...
        try {
            ...
            showAlert(Alert.AlertType.ERROR, "Announcements", "Inline update failed.\n" + e.getMessage());
        }
    }
    ...
    private void updateDetailPanel() {
        ...
        formHintLabel.setText(editing
                ? "Double-click table cells to update the selected announcement instantly."
