...
public class AdminUserModerationController {
    ...
    @FXML private BarChart<String, Number> accountStatusBarChart;
    @FXML private PieChart accountStatusPieChart;
    ...
    // ── NEW: face buttons (add these two in your FXML toolbar) ───────────────
    ...
    private FaceRecognitionService faceService;   // NEW
    ...
    public void initialize() {
        ...
        try {
            ...
            // ── NEW: boot face service and seed labels from all users ──────
            faceService = new FaceRecognitionService();
            userService.getAll().forEach(u ->
                    faceService.refreshLabels(Map.of(u.getId(), u.getEmail())));
            // ─────────────────────────────────────────────────────────────
            ...
            if (registerFaceBtn != null) registerFaceBtn.setDisable(true);
            if (deleteFaceBtn   != null) deleteFaceBtn.setDisable(true);
            showStatus("User moderation is unavailable because the database connection failed.", "status-error");
        } catch (SQLException ex) {
            // Face label pre-load failed — non-fatal, recognition still works
            // but will show "user-{id}" placeholders until next restart
        }
    }
    ...
    private void handleResetFilters() {
        searchField.clear();
        if (sortComboBox != null) sortComboBox.setValue(SORT_A_TO_Z);
        applyFilter();
    ...
    private void handleSave() {
        hideValidation();
        if (selectedUser == null) { showValidation("Select a user before saving changes."); return; }
        if (userService == null)  { showValidation("The user moderation service is not available."); return; }
        ...
        if (email == null || prenom == null || nom == null) {
            showValidation("Email, first name, and last name are required."); return;
        }
        if (status == null) { showValidation("Status is required."); return; }
        if (newPassword != null && !newPassword.isBlank()) {
            if (newPassword.length() < 8) {
                showValidation("New passwords must contain at least 8 characters."); return;
            }
            if (!Objects.equals(newPassword, confirmPassword)) {
                showValidation("Password confirmation does not match."); return;
            }
        }
        ...
        if (currentUser != null && currentUser.getId() != null
            ...
            showValidation("You cannot remove your own admin access."); return;
        }

        try {
            if (userService.emailExists(email, selectedUser.getId())) {
                showValidation("Another user already uses this email address."); return;
            }
            ...
            // Keep face label map in sync when email changes
            if (faceService != null)
                faceService.refreshLabels(Map.of(selectedUser.getId(), selectedUser.getEmail()));
    ...
    private void handleDelete() {
        hideValidation();
        if (selectedUser == null) { showValidation("Select a user before deleting."); return; }
        if (userService == null)  { showValidation("The user moderation service is not available."); return; }

        User currentUser = AuthSession.getCurrentUser();
        if (currentUser != null && currentUser.getId() != null
                && currentUser.getId().equals(selectedUser.getId())) {
            showValidation("You cannot delete the account you are currently using."); return;
        }
        ...
        try {
            ...
            // Clean up face data for the deleted user
            if (faceService != null) faceService.deleteFace(deletedId);
    ...
    /**
     * Opens the face registration modal for the currently selected user.
     * The admin selects a user in the table, then clicks "Register Face".
     */
    @FXML
    private void handleRegisterFace() {
        ...
        try {
            ...
            FaceRegisterController ctrl = loader.getController();
            // setTargetUser MUST be called before showAndWait so the controller
            // receives the user while the FX thread is still in this method.
            ctrl.setTargetUser(selectedUser);
            ...
            modal.showAndWait();   // blocks here; setTargetUser already ran above

            // After the modal closes, refresh labels so the recogniser uses
            // the latest email in case it was changed before registration
            if (faceService != null)
                faceService.refreshLabels(Map.of(selectedUser.getId(), selectedUser.getEmail()));
            ...
            showValidation("Could not open face registration: " + e.getMessage());
        }
    }

    /**
     * Deletes all stored face samples and the LBPH model entry for the
     * currently selected user.  Requires confirmation.
     */
    @FXML
    private void handleDeleteFace() {
        ...
        if (faceService == null) {
            showValidation("Face service is not available.");
            return;
        }
        if (!faceService.isFaceRegistered(selectedUser.getId())) {
            showValidation(selectedUser.getDisplayName() + " has no face data registered.");
            return;
        }
        ...
        confirm.setContentText("The user will no longer be able to log in with their face.");
        if (confirm.showAndWait().orElse(ButtonType.CANCEL) != ButtonType.OK) return;

        boolean ok = faceService.deleteFace(selectedUser.getId());
        if (ok) {
            showStatus("Face data deleted for " + selectedUser.getDisplayName(), "status-success");
        } else {
            showValidation("Face data could not be fully deleted — check the logs.");
        }
    }

    // ── Private helpers (all unchanged) ──────────────────────────────────────

    private void configureInputs() {
        roleComboBox.setItems(FXCollections.observableArrayList(UserRoles.allowedRoles()));
        statusComboBox.setItems(FXCollections.observableArrayList(
                "ACTIVE", "INACTIVE", "BLOCKED", "PENDING", "ACTIF", "INACTIF"));
        ...
        if (dateNaissancePicker != null) dateNaissancePicker.setEditable(false);
        clearForm();
        hideValidation();
    }
    ...
    private void bindUi() {
        ...
        userTableView.getSelectionModel().selectedItemProperty()
                .addListener((obs, o, n) -> populateForm(n));
        if (sortComboBox != null)
            sortComboBox.valueProperty().addListener((obs, o, n) -> applyFilter());
    }
    ...
    private void refreshUsers(Integer preferredUserId) {
        ...
        try {
            ...
            applyFilter();
            updateMetrics();
            if (preferredUserId != null) selectUserById(preferredUserId);
    ...
    private void updateMetrics() {
        ...
        long inactiveCount = users.stream().filter(u -> !u.isActiveAccount()).count();
    ...
    private void updateCharts() {
        List<User> visible = new ArrayList<>(sortedUsers);
        long activeCount  = visible.stream().filter(u -> "ACTIVE".equals(normalizeStatus(u.getStatut())) || "ACTIF".equals(normalizeStatus(u.getStatut()))).count();
        long blockedCount = visible.stream().filter(u -> "BLOCKED".equals(normalizeStatus(u.getStatut()))).count();
        long otherCount   = Math.max(0, visible.size() - activeCount - blockedCount);
        ...
        if (accountStatusPieChart != null) {
            ...
            if (activeCount  > 0) chartData.add(new PieChart.Data("Active ("  + activeCount  + ")", activeCount));
            if (blockedCount > 0) chartData.add(new PieChart.Data("Blocked (" + blockedCount + ")", blockedCount));
            if (otherCount   > 0) chartData.add(new PieChart.Data("Other ("   + otherCount   + ")", otherCount));
            if (chartData.isEmpty()) chartData.add(new PieChart.Data("No users", 1));
            accountStatusPieChart.setData(chartData);
        }

        if (chartSummaryLabel != null)
            chartSummaryLabel.setText(visible.size() + " visible user(s) | " + activeCount + " active | " + blockedCount + " blocked");
    }
    ...
    private void populateForm(User user) {
        ...
        // Update face button states whenever selection changes
        boolean hasFace = user != null && faceService != null && faceService.isFaceRegistered(user.getId());
        if (registerFaceBtn != null) registerFaceBtn.setDisable(user == null);
        if (deleteFaceBtn   != null) deleteFaceBtn.setDisable(!hasFace);

        if (user == null) {
            selectionStateLabel.setText("Select a user");
            emailField.clear(); prenomField.clear(); nomField.clear();
            ...
            photoField.clear(); cvField.clear();
    ...
    private void clearForm() {
        selectedUser = null;
        if (userTableView != null) userTableView.getSelectionModel().clearSelection();
        populateForm(null);
    }
    ...
    private Comparator<User> resolveUserComparator(String sortMode) {
        ...
        return SORT_Z_TO_A.equals(sortMode) ? c.reversed() : c;
    }
    ...
    private Path choosePdfTarget() {
        ...
        File f = chooser.showSaveDialog(
                userTableView == null || userTableView.getScene() == null
                        ? null : userTableView.getScene().getWindow());
    ...
    private void openFile(Path path) {
        ...
        if (Desktop.isDesktopSupported()) {
            try { Desktop.getDesktop().open(path.toFile()); return; }
    ...
    private void applyBarColor(XYChart.Data<String, Number> data, String color) {
        ...
        else data.nodeProperty().addListener((obs, o, n) -> styler.run());
    }

    private String emptyIfNull(String value)                  { return emptyIfNull(value, ""); }
    private String emptyIfNull(String value, String fallback) { return value == null ? fallback : value; }
}