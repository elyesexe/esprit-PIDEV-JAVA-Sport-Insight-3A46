...
public final class SchemaMigration {
    ...
    private static void ensureAnnonceSchema(Connection connection) throws SQLException {
        ...
        try (Statement statement = connection.createStatement()) {
            ...
            try {
                ...
                // Ignore best-effort backfill failures on legacy datasets.
            }
        }
    }
    ...
    private static void ensureFaceIdSchema(Connection connection) throws SQLException {
        ...
        try (Statement statement = connection.createStatement()) {
            ...
            addColumnIfMissing(metaData, catalog, statement, "face_profile", "model_name",
                    "VARCHAR(120) NOT NULL DEFAULT 'face_recognition'");
    ...
    private static void ensureNotificationSchema(Connection connection) throws SQLException {
        DatabaseMetaData metaData = connection.getMetaData();
        String catalog = currentCatalog(connection);
        try (Statement statement = connection.createStatement()) {
            if (!tableExists(metaData, catalog, "notification")) {
                statement.executeUpdate("""
                        CREATE TABLE notification (
                            ...
                            user_id INT NOT NULL,
                            source_annonce_id INT NULL,
                            source_user_id INT NULL
                        )
                        """);
            }

            addColumnIfMissing(metaData, catalog, statement, "notification", "type",
                    "VARCHAR(64) NOT NULL DEFAULT 'GENERAL'");
            addColumnIfMissing(metaData, catalog, statement, "notification", "created_at",
                    "DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP");
            addColumnIfMissing(metaData, catalog, statement, "notification", "is_read",
                    "BOOLEAN NOT NULL DEFAULT FALSE");
            addColumnIfMissing(metaData, catalog, statement, "notification", "user_id",
                    "INT NOT NULL");
            addColumnIfMissing(metaData, catalog, statement, "notification", "source_annonce_id",
                    "INT NULL");
            addColumnIfMissing(metaData, catalog, statement, "notification", "source_user_id",
                    "INT NULL");

            addIndexIfMissing(metaData, catalog, statement, "notification", "idx_notification_user",
                    "CREATE INDEX idx_notification_user ON notification (user_id)");
            addIndexIfMissing(metaData, catalog, statement, "notification", "idx_notification_read",
                    "CREATE INDEX idx_notification_read ON notification (is_read)");
            addIndexIfMissing(metaData, catalog, statement, "notification", "idx_notification_type",
                    "CREATE INDEX idx_notification_type ON notification (type)");
            addIndexIfMissing(metaData, catalog, statement, "notification", "idx_notification_created_at",
