...
The JavaFX app is validated with the Symfony web app on the same local machine.

- Symfony repository: `C:\final\sport_insight_final`
- shared database: `sport_insight`
- XAMPP MySQL port: `3306`
- Java launcher: `tn.esprit.gui.HomeLauncher`

Both apps must point to the same MySQL database before the integration demo. The Java app owns several support tables and columns used for desktop-only features such as face login, password reset, live notifications, football-data enrichment, and nutrition tracking. Avoid destructive Symfony schema-update commands on the shared database; use additive SQL/migrations only.
